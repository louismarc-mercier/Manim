...
class ThumbnailEnglish(Scene):
    ...
    def construct(self):
        line_e = TextMobject(r"\sc{How}", r"\sc{do you}", r"\sc{stay}", r"\sc{positive?}").scale(2.5).shift(2.25 * UP + 4 * LEFT).set_color(PINK)
        line_e[0].next_to(3 * UP + 7 * LEFT)
        line_e[1].next_to(2 * UP + 7 * LEFT)
        line_e[2].next_to(1 * UP + 7 * LEFT)
        line_e[3].next_to(0 * UP + 7 * LEFT)
        line_pi = TextMobject(r"{\sc It's in }", r"{\sc my nature!}").scale(2.5).set_color(YELLOW)
...
class Menu(Scene):
    def construct(self):
        ...
        l = NumberedList(
            *"""1) Mathematic applications, 2) Definition of power and exponent, 3) Properties of exponents, 4) Examples""".split(","), dot_color=BLUE
        )
...
class Applications(Scene):
    def construct(self):
        ...
        l = NumberedList(
            *"""1) Matrices (product of matrices), 2) Functions (for the composition), 3) Sets (for the cartesian product)""".split(","), dot_color=BLUE
        )
...
class Application_reg(Scene):
    def construct(self):
        title = TextMobject(r"\underline{ \textbf{Application: Polynomial Regression}}", color=PURPLE).scale(1.25)
        title.next_to(3*UP + 6.5*LEFT)

        path = r"C:/Users/Utilisateur/Desktop/CEGEP/MANIM/manim-recent/media/images/polynomial_reg_english/"
        nuage_points = ImageMobject(path + "scatter_plot")
        nuage_points.scale(2.5)
        nuage_points.next_to(7*LEFT + 0.75*DOWN)

        reg_lin_points = ImageMobject(path + "linear_reg_plot.png")
        reg_lin_points.scale(2.5)
        reg_lin_points.next_to(7 * LEFT + 0.75 * DOWN)

        poly_reg_points = ImageMobject(path + "poly_reg_plot.png")
        poly_reg_points.scale(2.5)
        poly_reg_points.next_to(7 * LEFT + 0.75 * DOWN)

        legende_1 = TextMobject(r"$y$", ": ",  r"Salary").scale(1)
        legende_1[0].set_color(BLUE)
        legende_1.next_to(1.5*RIGHT + 1.5*UP)

        legende_2 = TextMobject(r"$x$", ": ",  r"Experience").scale(1)
...
class ExponentDef(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\textbf{The Exponents}}").scale(1).set_color(PURPLE)
        context = TextMobject(r"The power operation consists in multiplying an element $a$ by itself "
                              r"multiple times. The number of factors involved in this operation is called ",
                              r"exponent ",
                              r"of the element $a$. Let $a\in\mathbb{R}$, then we have that ").scale(0.7)
        ...
        brace_txt = TextMobject(r"$n$", " times").scale(1.1)
        brace_txt[0].set_color(YELLOW)
        brace_txt.next_to(brace1,DOWN)

        interpretation = TextMobject(r"""
                        \begin{itemize}
                        \item $a$ raised to the $n$-th power,
                        \item $a$ raised to the power of $n$. 
                        \end{itemize}
                        """).scale(0.7)
...
class CasParticuliers(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\textbf{Particular Cases}}").to_corner(UL).scale(1).set_color(PURPLE)
        context = TextMobject(r"Let $a\in\mathbb{R}$, then we have that ")
        ...
        nom_1 = TextMobject(r"", r"(Area of a square)").scale(1).set_color(RED)
        nom_2 = TextMobject(r"", r"(Volume of a cube)").scale(1).set_color(BLUE)
        nom_3 = TextMobject(r"", r"(Convention)").scale(1).set_color(YELLOW)
        nom_4 = TextMobject(r"", r"(If $a$ is invertible)").scale(1).set_color(GREEN)
...
class ExtensionExpNegatifs(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\textbf{Extension to Negative Exponents}}").to_corner(UL).scale(1).set_color(PURPLE)
        context = TextMobject(r"If $a$ is invertible, we note $a^{-1}$ as the inverse.").scale(0.75)
        context.next_to([-6.75, 2.5, 0])

        context_2 = TextMobject2(r"If $n\in\mathbb{N}_{\star}$, then $a^{n}$ is also invertible and we note $a^{-n}$ as its inverse.").scale(0.75)
        context_2.next_to(context, DOWN, buff=LARGE_BUFF).align_to(context, LEFT)

        exemple_title = TextMobject2(r"\underline{Example:}").scale(1)
...
class Proprietes(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\textbf{Properties of Exponents}}").to_corner(UL).scale(1).set_color(PURPLE)
...
class Exemples(Scene):
    def construct(self):
        ...
        answers = TextMobject("Solutions (respectively): $2^{5}$, $7^{6}$, $\\frac{81}{8}$, $\\frac{7}{6}$").to_corner(DL)
...
class Conclusion(Scene):
    ...
    def construct(self):
        ...
        compl_rules_str = [
            "",
            "Definition of the power and exponent,",
            "Properties of integers exponents,",
            "Be able to simplify expressions with the properties of exponents.",
        ]
