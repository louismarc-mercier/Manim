...
class Menu(Scene):
    def construct(self):
        ...
        l = NumberedList(
            *"""1) Revision of axioms, 2) Factoring out common factors, 3) The grouping method,""".split(","), dot_color=BLUE
        )
...
class Distributivite(Scene):
    def construct(self):
        ...
        title = TextMobject(r"\underline{\sc Distributivity}", color=PURPLE).to_corner(UL).scale(1.25)

        context = TextMobject(r"For $a,b$ and $c\in\mathbb{R}$:")
        context.scale(1)
        ...
        titre_exemple = TextMobject(r"\underline{\sc Examples}", color=PURPLE)
...
class DistributiviteSuite(Scene):
    def construct(self):
        ...
        title = TextMobject(r"\underline{\sc Distributivity (cont'd)}", color=PURPLE).to_corner(UL).scale(1.25)
        title.move_to(3.25 * LEFT + 3 * UP)

        context = TextMobject(r"For $a,b,c$ and $d\in\mathbb{R}$:")
        context.scale(1)
        ...
        titre_exemple = TextMobject(r"\underline{\sc Example}", color=PURPLE)
        titre_exemple.move_to(5.75 * LEFT + 1 * DOWN)
...
class DistributiviteSuitePk(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Proof:}", color=PURPLE).to_corner(UL).scale(1.25)
        ...
        substitution_4 = TextMobject(r"(By distributivity)")
...
class Commutativite(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Commutativity of multiplication}", color=PURPLE).to_corner(UL).scale(1.25)
        title.move_to(1 * LEFT + 3 * UP)

        context = TextMobject(r"For $a$ and $b\in\mathbb{R}$:")
        context.scale(1)
        context.move_to(5* LEFT + 2*UP)
        #context.set_color(YELLOW)
...
class Symetrie(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Symetry}", color=PURPLE).to_corner(UL).scale(1.25)
...
class DefFactorisation(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Definining Factorization}", color=PURPLE).to_corner(UL).scale(1.25)
        title.move_to(3 * LEFT + 3 * UP)

        #definition = TextMobject2(r"Factoring consists of writing an algebraic expression in the form of a product of factors.")
        #definition.scale(0.7)
        #definition.move_to(1* LEFT + 2*UP)
        #definition.set_color(YELLOW)

        definition = TextMobject2(r"Factoring consists of writing an", r" algebraic expression",
                                  r" in the form of a ",
                                  r"product of factors", r".")
        ...
        pause = TextMobject("What axiom should you use to expand an expression?").next_to(circ, RIGHT)
        ...
        answer = TextMobject("Answer: Distributivity.").to_corner(DL)
        ...
        #circ = Arc(start_angle=PI / 2, angle=-2 * PI, radius=0.35).to_corner(DL)
        #timers = [TexMobject(str(i)).move_to(circ) for i in range(5, -1, -1)]
        #pause = TextMobject("What is the operation to factor an expression?").next_to(circ, RIGHT)
        #self.play(ShowCreation(circ), Write(pause))
        #self.play(Write(timers[0]), run_time=0.5)
        #for i in range(5):
        #    self.play(ReplacementTransform(timers[i], timers[i + 1]), run_time=0.5)
...
class MiseEvidenceSimple(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Factoring Out Common Factors}", color=PURPLE).to_corner(UL).scale(1.25)
        title.move_to(0 * LEFT + 3 * UP)

        context = TextMobject(r"It is a process which allows to extract a common multiplicative term from a sum of algebraic expressions.")
        context.scale(0.7)
        ...
        exemple_2 = TextMobject(r"$-2x^{2}+yx=x(-2x+y)$")
        exemple_2.scale(1)
...
class PourquoiMES(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Explanation: Factoring Out Common Factors}", color=PURPLE).to_corner(UL).scale(1.25)
        title.move_to(0 * LEFT + 3 * UP)
        title.scale(0.85)

        context = TextMobject(r"This is a consequence of the axiom of distributivity.")
        context.scale(0.85)
        context.move_to(0* LEFT + 2*UP)
        context.set_color(YELLOW)
        ...
        texte_distributivite = TextMobject(r"(Distributivity)")
        ...
        texte_mes = TextMobject(r"(By symetry)")
        texte_mes.scale(1)
...
class MiseEvidenceDouble(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc The Grouping Method}", color=PURPLE).to_corner(UL).scale(1.25)
        ...
        titre_exemple = TextMobject(r"\underline{\sc Idea}", color=PURPLE)
        titre_exemple.move_to(6.25 * LEFT + 1 * DOWN)
        exemple_1 = TextMobject(r"The idea of the grouping method is to factoring out common factors two times.")
        exemple_1.scale(0.75)
...
class Recette(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Recipe for the grouping method}", color=GOLD).to_corner(UL)
        definition = TextMobject(r"""\RedBox[label=exsecond]{}{
                                     \begin{enumerate}
                                     \item Group terms with common factors.
                                     \item For each group, factor out common factors.
                                     \item Factor out the common term. 
                                     \end{enumerate}
                                    }      
                                """
                                 )
...
class RecetteExemple(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Example:}", color=PURPLE).to_corner(UL).scale(1.25)
        title.move_to(5 * LEFT + 3 * UP)
        ...
        substitution_2 = TextMobject(r"(Step 1)")
        substitution_2.scale(0.8)
        substitution_2.move_to(5 * RIGHT + 0 * UP)

        substitution_3 = TextMobject(r"(Step 2)")
        substitution_3.scale(0.8)
        substitution_3.move_to(5 * RIGHT + 1 * DOWN)
        ...
        substitution_4 = TextMobject(r"(Step 3)")
        substitution_4.scale(0.8)
        substitution_4.move_to(5 * RIGHT + 2 * DOWN)
...
class FactoExoSup(Scene):
    ...
    def construct(self):
        title = TextMobject(r"\underline{\sc Additional exercises}").to_corner(UL).set_color(PURPLE).scale(1.15)
        exos = TextMobject(r" Factor the following expressions with the grouping method:",
        r"""
        \begin{enumerate}
        \item $ac-bc+a-b$,
        ...
        answer = TextMobject("Solutions (respectively): $(a-b)(c+1)$, $(y^{2}+4)(3x-y)$").to_corner(DL)
        self.play(Write(answer))
        self.wait(10)
...
class Conclusion(Scene):
    ...
    def construct(self):
        ...
        compl_rules_str = [
            "",
            "Factor with the \\textit{factoring out common factors} or \\textit{the grouping method}.",
            "Understanding the link between the factorization methods and distributivity.",
        ]
