...
class ThumbnailEnglish(Scene):
    ...
    def construct(self):
        line_e = TextMobject(r"\sc{We are}", r"\sc{so alike!}").scale(2.5).shift(2.25 * UP + 4 * LEFT).set_color([PINK,YELLOW])
        line_e[0].next_to(3 * UP + 7 * LEFT)
        line_e[1].next_to(2 * UP + 7 * LEFT)
        line_pi = TextMobject(r"{\sc I was like}", r"{\sc you before!}").scale(2.25).set_color([YELLOW,PINK])
...
class Exemple1Intro(Scene):
    ...
    def construct(self):
        title = BoxedTitle(text="Examples", width=3, height=0.7, scale=0.7)
        title.move_to(np.array([0, 3.25, 0]))

        # Get labels (names and values)
        names = ["Asterix", "Obelix", "$\dots$", "Panoramix"]
...
class MiseEnGarde(Scene):
    def construct(self):
        title = BoxedTitle(text="Caution", width=4, height=0.7, scale=0.7)
        title.move_to(np.array([0, 3.25, 0]))
        ...
        pause = TextMobject("What if $x<0$ ?").next_to(circ, RIGHT)
        ...
        answer = TextMobject("Answer: The function is not defined!").to_corner(DL)
...
class Formalisme(Scene):
    ...
    def construct(self):
        title = BoxedTitle(text="Formalism", width=3, height=0.7, scale=0.7)
        title.move_to(np.array([0, 3.25, 0]))

        statement = TextMobject2(r"$\textit{``$f$ is a function from $A$ to $B$''}$", " is expressed symbolically by:").scale(0.8)
        ...
        raw_dom_et_codom = [r"$\bullet$ $A$ corresponds to the domain of the function (i.e. $dom(f)$).",
                            r"$\bullet$ $B$ corresponds to the image of the function (i.e. $ima(f)$)."]
...
class Formalisme2(Scene):
    ...
    def construct(self):
        title = BoxedTitle(text="Formalism (cont'd)", width=4.5, height=0.7, scale=0.7)
        title.move_to(np.array([0, 3.25, 0]))

        statement = TextMobject2(r"$\textit{``$f$ transforms $x$ to $f(x)$''}$", " is expressed symbolically by:").scale(0.8)
        statement[0].set_color(BLUE)
        statement.next_to(title, DOWN, buff=LARGE_BUFF)
...
class Caracteristique(Scene):
    ...
    def construct(self):
        title = BoxedTitle(text="Characteristic", width=5, height=0.7, scale=0.7)
        title.move_to(np.array([0, 3.25, 0]))

        statement = TextMobject2(r"The value of $f(x)$ is set $\underline{\textbf{uniquely}}$ for any element $x$ of the domain.").scale(0.7)
        statement[0].set_color(BLUE)
        statement.next_to(title, DOWN, buff=LARGE_BUFF)

        contre_exemple_nom = TextMobject2(r"Counter-examples:").scale(0.7).next_to(statement, DOWN, buff=1*LARGE_BUFF).align_to(statement, LEFT)
        cercle = Circle().next_to(contre_exemple_nom, DOWN, buff=1*LARGE_BUFF).set_color(RED).align_to(statement, LEFT)
        contre_exemple_cercle = TextMobject2(r"Circle $\Big($e.g. $y=\pm\sqrt{5-x^{2}}\Big)$").scale(0.7).next_to(cercle, DOWN)

        ellipse = Ellipse(width=3, height=2).next_to(cercle, RIGHT, buff=3 * LARGE_BUFF).set_color(ORANGE)
        contre_exemple_ellipse = TextMobject2(r"Ellipse $\bigg($e.g. $y=\pm\sqrt{9-\frac{9x^{2}}{25}}\bigg)$").scale(0.7)
        contre_exemple_ellipse.next_to(ellipse, DOWN)
...
class Pratique(GraphScene):
    ...
    def construct(self):
        title = BoxedTitle(text="Exercise", width=5, height=0.7, scale=0.7)
        title.to_corner(UL)


        statement_1 = TextMobject2(r"Consider $f:A\rightarrow B$ defined by").scale(0.75)
        ...
        raw_dom_et_codom = [r"$\bullet$ What is the domain of $f$?", r"$\bullet$ What is the image of $f$?"]
        dom_et_codom = [TextMobject(elem).scale(0.75) for elem in raw_dom_et_codom]
