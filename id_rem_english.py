...
class Menu(Scene):
    def construct(self):
        ...
        l = NumberedList(
            *"""1) Motivation, 2) Remarkable Identities""".split(","), dot_color=BLUE
        )
...
class Motivation(Scene):
    def construct(self):
        ...
        l = NumberedList(
            *"""1) Accelerate computations, 2) Factorization, 3) Develop algebraic expressions""".split(","), dot_color=BLUE
        )
...
class IdentitesRemarquables(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Remarkables Identities}").to_corner(UL).scale(1)
        context = TextMobject(r"Let $a,b\in\mathbb{R}$, then we have that }")
        ...
        nom_1 = TextMobject(r"", r"RI1").scale(1).set_color(RED)
        nom_2 = TextMobject(r"", r"RI2").scale(1).set_color(BLUE)
        nom_3 = TextMobject(r"", r"RI3").scale(1).set_color(YELLOW)
...
class IdRem1(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Proof of RI1}", color=RED).to_corner(UL).scale(1)

        eq_1 = TextMobject(r"$(a+b)^{2}$", r" $=$", r" $(a+b)(a+b)$").scale(1.1)
        justification_1 = TextMobject(r"(By definition)").scale(0.9)
        eq_1.next_to([-6, 1.5, 0], RIGHT)
        justification_1.next_to([2.75, 1.35, 0])

        eq_2 = TextMobject(r" $=$",r"$a^{2}+ab+ba+b^{2}$").scale(1.1)
        justification_2 = TextMobject(r"(By distributivity)").scale(0.9)
        ...
        justification_3 = TextMobject(r"(By commutativity)").scale(0.9)
...
class IdRem2(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Proof of RI2}", color=BLUE).to_corner(UL).scale(1)

        eq_1 = TextMobject(r"$(a-b)^{2}$", r" $=$", r" $(a+c)^{2}$").scale(1.1)
        justification_1 = TextMobject(r"(Let $c=-b$)").scale(0.9)
        eq_1.next_to([-6, 1.5, 0], RIGHT)
        justification_1.next_to([2.75, 1.35, 0])

        eq_2 = TextMobject(r" $=$",r"$a^{2}+2ac+c^{2}$").scale(1.1)
        justification_2 = TextMobject(r"(By RI1)").scale(0.9).set_color(RED)
        eq_2[0].next_to(eq_1, DOWN, buff=LARGE_BUFF).align_to(eq_1[1], LEFT)
        eq_2[1].next_to(eq_2[0], RIGHT, buff=LARGE_BUFF).align_to(eq_1[2], LEFT)
        justification_2.next_to(eq_2[1], RIGHT, buff=LARGE_BUFF).align_to(justification_1, RIGHT)

        eq_3 = TextMobject(r" $=$", r"$a^{2}+2a(-b)+(-b)^{2}$").scale(1.1)
        justification_3 = TextMobject(r"(By definition)").scale(0.9)
        eq_3[0].next_to(eq_2, DOWN, buff=LARGE_BUFF).align_to(eq_2[0], LEFT)
        eq_3[1].next_to(eq_3[0], RIGHT, buff=LARGE_BUFF).align_to(eq_2[1], LEFT)
        justification_3.next_to(eq_3[1], RIGHT, buff=LARGE_BUFF).align_to(justification_2, RIGHT)
...
class IdRem3(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Proof of IR3:}", color=YELLOW).to_corner(UL).scale(1)

        eq_1 = TextMobject(r"$(a-b)(a+b)$", r" $=$", r" $a^{2}+ab-ba-b^{2}$").scale(1.1)
        justification_1 = TextMobject(r"(Distributivity)").scale(0.8)
        eq_1.next_to([-6, 1.5, 0], RIGHT)
        justification_1.next_to([2.75, 1.35, 0])

        eq_2 = TextMobject(r" $=$ ", r"$a^{2}$", r" $+$ ", r"$(ab$", r" $-$ ", r"$ab)$", r" $-$ ", r"$b^{2}$").scale(1.1)
        justification_2 = TextMobject(r"(Commutativity)").scale(0.8)
...
class MeasuredIdentity(Scene):
    ...
    def pre_square(self):
        ...
        title = TextMobject(r"\sc Proof (geometric): ", color=WHITE).to_corner(UL)
...
class Exercises1(Scene):
    ...
    def construct(self):
        ...
        exos = TextMobject(r" Factor the following expressions using remarkable identities:",
        r"""
        \begin{enumerate}
        ...
        answer = TextMobject("Solutions (respectively): $(\sqrt{a}-\sqrt{b})^{2}$, $(4y-2x)(4y+2x)$").to_corner(DL)
