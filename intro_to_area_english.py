...
class AreaThumbnail(Scene):
    ...
    def construct(self):
        ...
        line2 = TextMobject(r"Area").scale(3).next_to(line1, DOWN).set_color(BLUE)
...
class AreaDefinition(Scene):
    ...
    def construct(self):
        ...
        definition = TextMobject(r"The area is the surface occupied by an object on a two-dimensional plane. The area is calculated in 'square' units.")
...
class Conclusion(Scene):
    ...
    def construct(self):
        title = TextMobject(r"\underline{\sc To remember}", color=WHITE).to_corner(UL)

        compl_rules_str = [
            "",
            "The area is the surface occupied by an object on a two-dimensional plane.",
            "The area is calculated in squared units.",
            "The area of a bounded surface is positive.",
            "Given a unit length, the area of a square of side 1 is equal to 1.",
            "The area is additive.",
            "The area is unvariant to translations and rotations."
        ]
...
class Applications(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Concrete applications}", color=WHITE).to_corner(UL)

        compl_rules_str = [
            "",
            "Buying windows (price per square centimeters).",
            "Paint some walls (price per liter of paint).",
            "Install grass on a field.",
        ]
...
class AreaProperties(Scene):
    def construct(self):
        title = TextMobject(r"\underline{\sc Area Properties}", color=WHITE).to_corner(UL)

        compl_rules_str = [
            "",
            "The area of a bounded surface is positive.",
            "Given a chosen unit length, the area of a square of side 1 is equal to 1.",
            "The area is additive.",
            "The area is unvariant to translations and rotations."
        ]

        rules = [TextMobject("P{}) {}".format(i, rule), color=YELLOW) for i, rule in enumerate(compl_rules_str)]
        rules = [rule.scale(0.7) for rule in rules]
...
class SquareDecompositionP3(Scene):
    ...
    def pre_square(self):
        ...
        title = TextMobject(r"\sc Example (P3): Area Additivity (square, $c=3$)", color=WHITE).to_corner(UL)
...
class ExampleSquareArea(Scene):
    ...
    def pre_square(self):
        ...
        vert_measure = MeasureDistance(Line(dl, dr), invertir=True, dashed=True, buff=-0.25).add_tips()\
            .add_tex("3\\text{ units}", buff=-3.7, color=WHITE)
        hor_measure = MeasureDistance(Line(dl, ul), invertir=False, dashed=True, buff=0.5).add_tips()\
            .add_tex("3\\text{ units}", buff=2, color=WHITE)
        measures_1 = VGroup(hor_measure, vert_measure)


        title = TextMobject(r"\sc Example: Square", color=WHITE).to_corner(UL)
        self.title = VGroup(title)
        self.play(Write(title, run_time=1), ShowCreation(square, run_time=1),
                  *[GrowFromCenter(object) for object in [*measures_1]], run_time=1
                  )
        ...
        line1 = TextMobject(r"Aire","=","$3\\text{ units} \\times 3\\text{ units}$","=","$9\\text{ units}^{2}$").\
...
class UnitSquare(Scene):
    ...
    def pre_square(self):
        ...
        title = TextMobject(r"\sc P2) Basic Figure: Unit Square ($c=1$)", color=WHITE).to_corner(UL)
        self.title = VGroup(title)
        self.play(Write(title, run_time=1), ShowCreation(square, run_time=1),
                  *[GrowFromCenter(object) for object in [*measures_1]], run_time=1
                  )
...
class RotateSquare(Scene):
    def construct(self):
        #circle = Circle()
        title = TextMobject(r"\sc Example (P4): Invariance to rotations (carré)", color=WHITE).to_corner(UL)
        circle = Square()
...
class RotateRectangle(Scene):
    def construct(self):
        title = TextMobject(r"\sc Example (P4): Invariance to rotations ", color=WHITE).to_corner(UL)
        ...
        for i, rotated_rectangle in enumerate(rotated_rectangles):
...
class TranslateRectangle(Scene):
    def construct(self):
        title = TextMobject(r"\sc Example (P4): Invariance to translations", color=WHITE).to_corner(UL)
        title.scale(0.95)
        initial_rectangle = Rectangle()
        ...
        current_rectangle = initial_rectangle
        self.wait(10)
        for i, translated_rectangle in enumerate(translated_rectangles):
...
class ShrinkSquare(Scene):
    def construct(self):
        title = TextMobject(r"\sc Example (P1): Positive area (square, $c>0$)", color=WHITE).to_corner(UL)
...
class Credits(Scene):
    ...
    def construct(self):
        ...
        instructor = TexMobject(r"\text{Instructor}", r"\text{Louis-Marc Mercier}")
